...
import { CheckCircle, AlertTriangle, RefreshCw, Eye, ScanSearch, FileText, X, Image as ImageIcon } from 'lucide-react';
...
export const Scanner = () => {
    ...
    return (
        <div className="p-4 sm:p-6 md:p-8 flex-1 flex flex-col space-y-4 sm:space-y-6">
            <Topbar
                ...
                <div className="lg:col-span-5 bg-white border border-slate-200 rounded-2xl p-4 sm:p-6 shadow-sm flex flex-col gap-4 min-h-[320px] sm:min-h-[400px]">
                    ...
                    {!isLoading && scanResult && scannerMode === 'label' && (
                        <div className="space-y-3 sm:space-y-4">
                            ...
                            <div className="bg-slate-50 p-3.5 sm:p-4 rounded-xl border border-slate-100 space-y-1.5 sm:space-y-2">
                    ...
                    {!isLoading && certResult && scannerMode === 'cert' && (
                        <div className="space-y-3 sm:space-y-4">
                            <div className={`p-3.5 sm:p-4 rounded-xl border ${
                                certResult.status === 'Valid' ? 'bg-green-50 border-green-200 text-green-800' : 'bg-red-50 border-red-200 text-red-800'
                            }`}>
                                <span className="text-[10px] uppercase font-bold">Logo Authentication State</span>
                                <p className="text-base sm:text-lg font-black">{certResult.status}</p>
                            </div>
                            ...
                            <div className="bg-slate-50 p-3.5 sm:p-4 rounded-xl border border-slate-100 space-y-1.5 sm:space-y-2 text-xs">
                                ...
                                <p className="text-slate-600">Logo ID: <span className="font-bold text-slate-800 font-mono">{certResult.certificateNumber}</span></p>
