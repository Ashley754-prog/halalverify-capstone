...
const MetricCard = ({ label, value, unit, sublabel, color = 'emerald', icon }) => {
    ...
    return (
        <div className={`bg-gradient-to-br ${colors[color]} rounded-2xl p-6 text-white shadow-lg`}>
            ...
            <p className="text-4xl font-black tracking-tight">{value}<span className="text-lg font-semibold ml-1 text-white/80">{unit}</span></p>
            ...
            <table className="w-full text-sm">
                <thead>
                    <tr className="bg-slate-50 border-b border-slate-100 text-xs uppercase tracking-wider text-slate-500">
                        ...
                        <th className="px-6 py-3 text-left font-bold">mAP₅₀ (%)</th>
                ...
                <tbody className="divide-y divide-slate-100">
                    {batches.map((b, i) => (
                        <tr key={i} className="hover:bg-slate-50 transition-colors">
                            <td className="px-6 py-4 font-semibold text-slate-800">{b.batch}</td>
                            <td className="px-6 py-4 text-slate-600">{b.precision.toFixed(1)}</td>
                            <td className="px-6 py-4 text-slate-600">{b.recall.toFixed(1)}</td>
                            <td className="px-6 py-4 font-bold text-emerald-700">{b.mAP50.toFixed(1)}</td>
        ...
        <div className="p-8 space-y-8">
            ...
            <div>
                <div className="flex items-center gap-2 mb-4">
                    <span className="text-xs font-bold uppercase tracking-widest text-slate-400">Module 1</span>
                    <div className="h-px flex-1 bg-slate-200" />
                    <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-200">YOLOv8-Nano + EasyOCR</span>
                </div>
            ...
            <div>
                <div className="flex items-center gap-2 mb-4">
                    <span className="text-xs font-bold uppercase tracking-widest text-slate-400">Module 2</span>
                    <div className="h-px flex-1 bg-slate-200" />
                    <span className="text-xs font-bold text-purple-600 bg-purple-50 px-3 py-1 rounded-full border border-purple-200">PaddleOCR Structure</span>
                </div>
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <MetricCard label="Field Extraction Rate" value={paddleocr.fieldExtractionRate} unit="%" sublabel="Correctly extracted cert fields" color="blue" icon={<Target size={18} />} />
                </div>
            </div>
            ...
            <div>
                <div className="flex items-center gap-2 mb-4">
                    <span className="text-xs font-bold uppercase tracking-widest text-slate-400">Usability Evaluation</span>
                    <div className="h-px flex-1 bg-slate-200" />
                    <span className="text-xs font-bold text-amber-600 bg-amber-50 px-3 py-1 rounded-full border border-amber-200">System Usability Scale (SUS)</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="md:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col justify-center">
                        <p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-3">SUS Score Interpretation</p>
                        <div className="flex flex-col gap-2">
                            {[
                                ...
                                <div key={label} className="flex items-center gap-3">
                                    ...
                                    <span className={`text-sm font-medium ${label === 'Excellent' ? 'text-blue-700 font-bold' : 'text-slate-500'}`}>
                                        {range} — {label}
                                        {label === 'Excellent' && <span className="ml-2 text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">Current Score</span>}
                                    </span>
                                </div>
                            ))}
                        </div>
                        <p className="text-xs text-slate-400 mt-4">Based on {sus.respondents} respondents from Zamboanga City consumer/inspector group.</p>
                    </div>
                </div>
            </div>
        </div>
